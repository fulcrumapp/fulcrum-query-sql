...
const element_column_1 = __importDefault(require("./schema/element-column"));
const simple_column_1 = __importDefault(require("./schema/simple-column"));
class FormFieldSchema {
    constructor({ fullSchema = false }) {
        this.fullSchema = fullSchema;
    }
    addSystemColumn(label, attribute, columnName, type, accessor, join, sql) {
        const column = new simple_column_1.default({ name: label,
    ...
    addRawElementColumn(element, rawColumn, id, type, part, columnKey) {
        const columnObject = new element_column_1.default({ element, rawColumn, type, id, part, index: this._columns.length });
